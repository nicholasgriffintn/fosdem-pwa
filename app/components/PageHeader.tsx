...
export function PageHeader({ heading, text, className, children, displayHeading = true, breadcrumbs }: PageHeaderProps) {
  return (
    <>
        ...
        <ul className="flex flex-row gap-2 mb-4 text-sm text-muted-foreground">
          {breadcrumbs.map((breadcrumb, index) => (
            <li key={breadcrumb.href}>
              <Link to={breadcrumb.href} search={(prev) => ({ ...prev, ...breadcrumb.search })} className="no-underline">
                {breadcrumb.title}
              </Link>
