...
async function generateServiceWorker(outputDir = 'dist') {
  ...
  const assetsToCache = filesAndDataUrls
    ...
    // Sourcemaps are debugging aids, never requested by the running app. They
    // were 7.05 MB of an 8.86 MB precache — downloaded in full by every visitor.
    .filter(file => !file.endsWith('.map'))

  // Vite emits content-hashed filenames under /assets, so the URL already
  // identifies the exact bytes. Giving those entries a build-time revision made
  // Workbox treat every asset as changed on every deploy and re-download the
  // entire precache, even though nothing about them differed.
  const isContentHashed = (url: string) => /^\/assets\/.+-[A-Za-z0-9_-]{8,}\.[a-z0-9]+$/.test(url)

  const precacheEntries = assetsToCache.map(url => ({
    url,
    revision: isContentHashed(url) ? null : CACHE_NAME,
  }))

  const precachedUrls = new Set(precacheEntries.map(entry => entry.url))
  const precachedBytes = clientFiles
    .filter(file => precachedUrls.has(`/${file.replace(new RegExp(`^${outputDir}/`), '').replace(/^client\//, '')}`))
    .reduce((total, file) => total + statSync(file).size, 0)

  console.log(
    `Precaching ${precacheEntries.length} entries (${(precachedBytes / 1024 / 1024).toFixed(2)} MB), ` +
    `${precacheEntries.filter(entry => entry.revision === null).length} content-hashed`
  )
...
workbox.precaching.precacheAndRoute(${JSON.stringify(precacheEntries, null, 2)});
