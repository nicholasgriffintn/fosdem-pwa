...
import type { BookmarkAction } from "~/components/shared/types";
...
export const Route = createFileRoute("/speakers/$slug")({
	...
	loader: async ({ params, deps: { year } }) => {
		...
		if (person) {
			personEvents = Object.values(data.events).filter(
				(event) => isEvent(event) && event.personIds?.includes(person.id),
			);
		}
...
function SpeakerPage() {
	...
	const onCreateBookmark = async (bookmark: BookmarkAction) => {
		await createBookmark({ ...bookmark, year });
	};
		...
		<PageShell maxWidth="none">
			...
			{person.biography && (
				<div className="mt-8 prose prose-lg prose-indigo text-foreground">
					...
					<div className="mt-2">
						{sanitiseString(person.extended_biography || person.biography)}
					</div>
				</div>
			)}
