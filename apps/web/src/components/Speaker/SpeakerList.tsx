...
export function SpeakerList({ persons, year, initialQuery = "" }: SpeakerListProps) {
  ...
  if (!isClient) {
    return (
      <SpeakerListStatic persons={persons} year={year} initialQuery={initialQuery} />
    );
  }

  return <SpeakerListClient persons={persons} year={year} initialQuery={initialQuery} />;
}

function SpeakerListClient({ persons, year, initialQuery }: Required<SpeakerListProps>) {
...
function SpeakerListStatic({ persons, year, initialQuery }: Required<SpeakerListProps>) {
  const normalizedQuery = initialQuery.toLowerCase().trim();
  const filteredPersons = normalizedQuery
    ? persons.filter((person) => person.name.toLowerCase().includes(normalizedQuery))
    : [...persons];
