...
export function EventContent({ year, event, persons }: EventContentProps) {
	return (
		<>
			...
			{event.abstract && (
				<div id="abstract" className="mt-4 scroll-mt-32">
					<h2 className="text-xl font-medium text-foreground">Abstract</h2>
					<div className="mt-2 prose prose-lg prose-indigo mt-4 text-foreground">
						{sanitiseString(event.abstract)}
					</div>
				</div>
			)}
			{event.description && (
				<div id="description" className="mt-4 scroll-mt-32">
					<h2 className="text-xl font-medium text-foreground">Description</h2>
					<div className="mt-2 prose prose-lg prose-indigo mt-4 text-foreground">
						{sanitiseString(event.description)}
					</div>
				</div>
			)}
			{event.attachments?.length > 0 && (
				<div id="attachments" className="mt-4 scroll-mt-32">
			...
			{event.links?.length > 0 && (
				<div id="links" className="mt-4 scroll-mt-32">
			...
			{(event.url || event.feedbackUrl) && (
				<div id="external-links" className="mt-4 scroll-mt-32">
