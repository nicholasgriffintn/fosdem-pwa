import { getSyncQueue, removeFromSyncQueue, recordSyncFailure } from "~/lib/localStorage";
...
export async function syncNotesToServer(): Promise<SyncResult> {
  ...
  // Count the attempt against the queued item so a permanently-failing entry is
  // eventually dropped instead of retried on every reconnect for the whole
  // seven-day TTL.
